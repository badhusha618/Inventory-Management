...
public class SearchController {
    ...
    @GetMapping("/search")
    public ResponseEntity<List<Product>> getProductsBySearch(@ApiParam("Product Name") @RequestParam String productName) throws UnitedSuppliesException {
    ...
    public ResponseEntity<List<Product>> getProductsByFilter(@ApiParam("Product Filter Details") @PathVariable("categoryId") String categoryId, @RequestBody ProductFilterRequest productFilterRequest) throws UnitedSuppliesException {
        final Map<Long, UserCart> productUserCart = new HashMap<>();
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication != null && !(authentication instanceof AnonymousAuthenticationToken)) {
            UnitedSuppliesUserDetail userDetail = (UnitedSuppliesUserDetail) authentication.getPrincipal();
