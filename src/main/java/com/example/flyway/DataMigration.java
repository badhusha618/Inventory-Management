...
public class DataMigration implements FlywayMigrationStrategy {
    ...
    private final Logger LOG = LogManager.getLogger(DataMigration.class);
    ...
    private void prettyDisplay(final MigrationInfoService migrationInfo) {
        ...
        if (all.length != 0) {
            for (MigrationInfo info : all) {
                LOG.info("Version: {} Description: {} Type: {} Installed By: {} Script Name: {} Status: {}",
                        info.getVersion(), info.getDescription(), info.getType(), info.getInstalledBy(), info.getScript(), info.getState());
            }
        }
    }
